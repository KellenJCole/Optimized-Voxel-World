...
void main() {
    ...
    // Combine results
    vec4 tex = texture(textureArray, vec3(TexCoord, BlockType - 1));
    vec3 result = (ambient + diffuse) * tex.xyz;
    FragColor = vec4(result, tex.w); // Ensure the alpha value is set to 1 for full opacity
}